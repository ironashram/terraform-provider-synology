...
func (p *Project) ServicesWithBuild() []string {
	...
	return slices.AppendSeq(make([]string, 0, len(servicesBuild)), maps.Keys(servicesBuild))
}
...
func (p *Project) ServicesWithExtends() []string {
	...
	return slices.AppendSeq(make([]string, 0, len(servicesExtends)), maps.Keys(servicesExtends))
}
...
// It returns a new Project instance with the changes and keep the original Project unchanged.
func (p *Project) WithSelectedServices(
	names []string,
	options ...DependencyOption,
) (*Project, error) {
	newProject := p.deepCopy()
...
// It returns a new Project instance with the changes and keep the original Project unchanged.
func (p *Project) WithImagesResolved(
	resolver func(named reference.Named) (godigest.Digest, error),
) (*Project, error) {
...
func (p *Project) WithServicesTransform(
	fn func(name string, s ServiceConfig) (ServiceConfig, error),
) (*Project, error) {
	type result struct {
		name    string
	...
	for name, s := range p.Services {
		if s.ContainerName != "" {
			if existing, ok := names[s.ContainerName]; ok {
				return fmt.Errorf(
					`services.%s: container name %q is already in use by service %s"`,
					name,
					s.ContainerName,
					existing,
				)
